...
| **Negative Net Feed Margin (Glut Trap)** | ❌ CRITICAL | Feeding animals when `sale_price < wheat_price * interval`. Milk/wool crashed to $1 while wheat surged to $45+. | Implement macroeconomic demand guard: abstain from expansion / halt unprofitable feeding unless 2+ town demand buildings exist. Suppress rescue feed orders (`_v234_rescue`) when product price <= $10 and shops == 0. |
| **Oversupply Glut from Livestock Census Asymmetry** | ❌ CRITICAL | Expanding to 17 cows/sheep in Q4 when opponent already owns 4-9 livestock and town has <= 3 shops. Combined herd overwhelms shops, crashing prices to $1.00. | Enforce aggregate herd capacity formula: `(us + opp_animals) <= 4.5 * shops`. Fall back to Route 0 high-margin crops (Strawberry/Melon) when capacity exceeded. |
