...
def process_file(args):
    ...
    match_record['p0_crops_planted'] = dict(match_record['p0_crops_planted'])
    match_record['p1_crops_planted'] = dict(match_record['p1_crops_planted'])
    match_record['p0_animals_built'] = dict(match_record['p0_animals_built'])
    match_record['p1_animals_built'] = dict(match_record['p1_animals_built'])
    match_record['p0_action_counts'] = dict(match_record['p0_action_counts'])
    match_record['p1_action_counts'] = dict(match_record['p1_action_counts'])
    match_record['p0_sells'] = dict(match_record['p0_sells'])
    match_record['p1_sells'] = dict(match_record['p1_sells'])
...
if __name__ == '__main__':
    ...
    for folder_path, category in folders:
        ...
        for f in files:
            work_items.append((f, category))

    print(f"Starting parallel extraction of {len(work_items)} replay files using {cpu_count()} CPU cores...")
    with Pool(processes=min(cpu_count(), 16)) as pool:
        results = pool.map(process_file, work_items)

    all_matches = [r for r in results if r is not None]
    print(f"Extracted {len(all_matches)} matches in {time.time()-start_t:.2f} seconds.")
    ...
    print("Successfully exported JSON and CSV datasets.")
