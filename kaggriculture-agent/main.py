...
def build_plan(st):
    ...
    for p in PRODUCTS:
        ...
        reserve[p] = r
        # units we can still push above I0 at >= reserve, plus the town's future drain
        head[p] = units_sellable(p, st.minv[p], r, 900) + int(rate[p] * dl * P["opp_share"])

    plan = {"reserve": reserve, "head": head, "rate": rate}

    # ---- how much of each product is already in the pipeline -------------
        ...
        for x in range(st.n):
            ...
            if k == "WEED":
                ...
                if a is None:
                    ...
                    counts[a] += 1
                    ad = ANIMALS[a]
                    # first yield is `first` days after placement, not today
    ...
    # Animals sitting in the shed are already paid for: they must be housed and
    # fed whether or not we have cash left this turn.
    have = dict((k, counts[k] + st.shed.get(k, 0)) for k in ANIMALS)
    owned = have["GOOSE"] + have["COW"] + have["SHEEP"]

    # ---- budget cascade --------------------------------------------------
    # Feed money is untouchable: a starved animal is a total write-off.
    ...
    # ---- land: cheapest $/tile on the board, buy it early ----------------
    ...
    # ---- animal targets: whatever the market, the tiles and the cash allow
    want = dict(have)
    ...
    if st.day <= P["stop_animals_day"]:
        ...
        for kind, a in ANIMALS.items():
            ...
            if net <= 0:
                continue
            ranked.append((net / float(a["cost"] + feed), kind, per, a))
        ranked.sort(reverse=True, key=lambda z: z[0])
        for _, kind, per, a in ranked:
            room = head[a["prod"]] - pipeline[a["prod"]]
            by_market = int(room // per)
            unit_cost = a["cost"] + dl * wheat_price_buy * 0.45
            by_cash = int(budget // unit_cost)
            n = min(by_market, by_cash, tile_room, P["max_animals"] - total)
            if kind == "GOOSE":
                n = min(n, P["max_geese"] - have["GOOSE"])
            if n <= 0:
                continue
            want[kind] += n
            budget -= n * a["cost"]
    ...
    # ---- structures: house everything we already own, plus a little slack -
    need_coops = max(0, want["GOOSE"] - counts["GOOSE"] - coops_free)
    need_past = max(0, want["COW"] + want["SHEEP"] - counts["COW"]
                    - counts["SHEEP"] - pastures_free)
    slack = sum(st.shed.get(k, 0) for k in ANIMALS) + 2
    plan["build_coops"] = min(need_coops, slack, len(empties))
    plan["build_pastures"] = min(need_past, slack,
                                 max(0, len(empties) - plan["build_coops"]))

    # Build next to the shed: animals cost 3 actions a day, travel is the tax.
    ...
    # ---- crop mix for the remaining tiles --------------------------------
    free_for_crops = max(0, len(empties) - nb)
    feed_need = (owned + want["GOOSE"] + want["COW"] + want["SHEEP"] - owned) * dl \
        + owned * dl
    cand = []
    for crop in CROPS:
        pr = st.prices[crop]
        ...
        if room < units * 0.5:
            continue
        cand.append((profit / float(occ), crop, units, room))
    cand.sort(reverse=True, key=lambda z: z[0])
    crop_targets = {}
    left = free_for_crops
    seed_budget = budget

    # Feed first.  A wheat tile yields ~1/day for ~1.5 actions, which is the
    # cheapest feed there is once market wheat is bid up, and unlike every
    # other crop it competes for no market absorption at all.
    if owned > 0 and dl >= 4 and left > 0:
        need = int(math.ceil(owned * P["wheat_tiles_per_animal"])) \
            - crop_counts.get("WHEAT", 0)
        n = max(0, min(need, left, int(seed_budget // CROPS["WHEAT"]["seed"])))
        if n > 0:
            crop_targets["WHEAT"] = n
            seed_budget -= n * CROPS["WHEAT"]["seed"]
            left -= n
    for _, crop, units, room in cand:
        if left <= 0:
            break
        if crop in crop_targets:
            continue
        by_market = int(room // max(1, units)) + 1
        by_cash = int(seed_budget // CROPS[crop]["seed"])
        # seeds already bought are sunk: always plant them
    ...
    plan["plant_sites"] = [p for p in empties
                           if p not in plan["coop_sites"]
                           and p not in plan["pasture_sites"]]

    # ---- labour: hands are absurdly cheap, actions are worth $20-100 -----
    pending = plan["build_coops"] + plan["build_pastures"] + len(weeds)
    pending += sum(crop_targets.values())
    work = (animals_now * 2.9 + crop_tiles * 0.8 + pending * 1.7 + 14)
    ...
    plan["hands"] = max(0, min(n, by_work, P["max_hands"]))
    return plan
