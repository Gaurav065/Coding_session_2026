...
P = {
    ...
    "max_geese": 0,             # Keep 0
    "max_animals": 20,          # overall herd cap; mix between cows/sheep is EV-ranked
...
def estimate_opp_production(st, item):
    """Estimate opponent's daily production of an item from visible tiles"""
    prod = 0
...
def compute_exact_animal_ev(st, plan, kind, wheat_price_buy, proj_price, rate, opp_production, dl):
    ...
    opp_daily_supply = opp_production.get(prod, 0) / max(1, dl)
    town_drain = rate.get(prod, 1.0)
    prod_prices = []
    for pd in prod_days:
        days_remaining = dl - pd + 1
        price = projected_price(prod, st.minv.get(prod, 0), our_daily_supply, opp_daily_supply, days_remaining, town_drain, proj_price)
...
def compute_animal_mav_exact(st, plan, kind, a, dl, wheat_price_buy, proj_price, rate, opp_production):
    net_ev, total_acts, daily_cf, break_even = compute_exact_animal_ev(st, plan, kind, wheat_price_buy, proj_price, rate, opp_production, dl)
...
def build_plan(st):
    ...
    for p in PRODUCTS:
        opp_production[p] = estimate_opp_production(st, p)
        ...
        elif p in ("MILK", "WOOL", "EGG"):
            # N1: animal products throttle to CURRENT shop capacity plus a
            # PRODUCT-WEIGHTED look-ahead for future unlocks. Blended rate counts
            # all 8 future shops as if they were half-open the whole season,
            # which invites 15+ cows on day 0 against 435-unit "future" head;
            # observed winners run 8-10 cows total and grow the herd as shops
            # unlock. _EXPECTED_SHOP_RATE[p] weights this by how often the
            # product actually appears across the shop pool (MILK 2.25/day per
            # unlock, WOOL/EGG 1.5/day) - MILK gets more look-ahead than WOOL,
            # matching the winner mix of ~9 cows + 6 sheep.
            current_drain = rate_current[p] * dl
            look_ahead = _EXPECTED_SHOP_RATE[p] * max(0, dl - 3)
            headroom = max(0, int((current_drain + look_ahead) * 1.5) - int(opp_production[p]))
        else:
            blended_drain = rate[p] * dl
            if rate_current[p] >= 5.0 and p in ("STRAWBERRY", "TOMATO"):
                min_h = int(blended_drain * P.get("headroom_floor_frac", 0.80))
                headroom = max(min_h, int(blended_drain * 1.5) - int(opp_production[p]))
            else:
    ...
    for kind, a in ANIMALS.items():
        ...
        our_supply = per / max(1, dl) * (committed_count + 1)  # Gap 3: uses committed count
        ...
        # Use exact EV calculation
        exact = compute_animal_mav_exact(st, plan, kind, a, dl, wheat_price_buy, price, rate, opp_production)
    ...
    if st.day <= P["stop_animals_day"]:
        for item in animal_cand:
            action_val = item[0]
            # N2: removed the 20x/5x early-game animal boost. That multiplier
            # made cows/sheep dominate the ranking so completely that strawberry
            # (action_val ~102) never got budget before cows (99 * 20 = 1980)
            # exhausted it - which is why we never reached the 24-39 strawberry
            # estates observed in every replay winner. Raw action_val already
            # captures per-action EV correctly: MELON 118 > STRAWBERRY 102 >
            # COW 99 > SHEEP 95, matching the winner day-1 build order.
            # A mild 1.3x nudge keeps animals slightly preferred among ties.
            action_val *= 1.3 if dl >= 15 else 1.0
    ...
    for entry in global_cand:
        if entry[1] == "CROP":
            ...
            if kind == "COW": n = min(n, P["max_animals"] - have["COW"])
