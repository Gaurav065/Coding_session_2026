...
def agent(obs, config=None):
    ...
    # --- MACRO MARKET POLICY ---
    target_hands = min(12, 4 + int(cash / 200)) 
    hires = 0
    while len(hands_pos) + hires < target_hands and cash >= 10:
    ...
    # Sell policy (Dump all product)
    wheat_product = inventory.get("WHEAT", {}).get("product", 0)
    if wheat_product > 0:
        market_action.append(["SELL", "WHEAT", wheat_product])
