...
model = MacroAgentNet()
model.load_state_dict(torch.load("bc_shifted.pth", map_location="cpu", weights_only=True))
model.eval()
...
def nn_agent(obs, conf=None):
    ...
    # DO NOT HIDE MONEY! Let the agent use it!
...
def run_shadow_match(replay_path):
    ...
    while not g.done:
        ...
        try: act0 = nn_agent(g.get_observation(0))
        except: act0 = {"farmer": ["PASS"], "hands": [], "market": []}
...
for f in files[:5]:
    with open(f, "r") as tmp:
        ...
        if max(obs["farms"][0]["money"], obs["farms"][1]["money"]) > 100000:
            ...
            print(f"Shifted BC Net: ${res['new_our_money']:.0f} vs Ghost: ${res['new_opp_money']:.0f}")
