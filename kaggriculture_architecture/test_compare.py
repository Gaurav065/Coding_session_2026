...
def run_match(replay_path, agent_module):
...
for f in files:
    with open(f, "r") as tmp:
        ...
        if max(obs["farms"][0]["money"], obs["farms"][1]["money"]) > 100000:
            print(f"Testing on {os.path.basename(f)}...")
            
            res_their = run_match(f, their_agent)
            print(f"Their Agent: ${res_their['new_our_money']:.0f}")
            
            res_our = run_match(f, fieldbook_agent)
            print(f"Our Agent:   ${res_our['new_our_money']:.0f}")
            break
