...
# We will replace init_tape_tasks entirely
old_func = re.search(r"def init_tape_tasks\(\):.*?(?=def get_seat)", content, re.DOTALL).group(0)

new_func = """def init_tape_tasks():
    global _TAPE_TASKS
    ...
    for step, action in enumerate(trace_actions):
        ...
        # After hand moves, update hands_pos_offline for the NEXT step's collision checks!
