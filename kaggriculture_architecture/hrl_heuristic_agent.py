...
def agent(obs, config=None):
    ...
    # 3. Market Management (BUY SEEDS)
    # We do this AFTER hiring so we don't starve the workforce!
    for item in ["MELON", "WHEAT", "CARROT", "STRAWBERRY", "TOMATO", "COW", "SHEEP", "GOOSE"]:
        target = TARGET_PORTFOLIO["BUY_TARGETS"].get(item, 0)
        ...
        # Keep a massive $300 reserve to ensure we can hire hands to water these crops for the next 10 days!
        if current < target and cash >= cost + 300: 
            to_buy = min(target - current, int((cash - 300) / cost))
            if to_buy > 0:
