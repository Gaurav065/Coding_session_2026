...
class ResNetDay3Env(gym.Env):
    def __init__(self, steps_per_macro=24):
        super().__init__()
        self.steps_per_macro = steps_per_macro
        self.max_macro_steps = 3 # 3 macros * 24 = 72 steps (End of Day 3)
...
def load_pretrained_weights(model, path="custom_bc_master.pth"):
    ...
    for k, v in bc_dict.items():
        if "fusion.3" in k:
            mapped_dict[k.replace("fusion.3", "actor_head.0")] = v
        else:
            mapped_dict[k] = v
    for k, v in mapped_dict.items():
        if k in model_dict: model_dict[k] = v
...
def train_ppo():
    ...
    print(f"Running PPO Curriculum (Day 0-3) on {device}")
    ...
    torch.save(model.state_dict(), "ppo_resnet_day3.pth")
    print("Saved ppo_resnet_day3.pth")
