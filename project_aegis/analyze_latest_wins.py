...
wins_dir = r'C:\Users\GauravPatel\Downloads\aegis_latest\wins'
win_files = sorted([f for f in os.listdir(wins_dir) if f.endswith('.json')])

print("=" * 80)
print(f"AEGIS LATEST WINS FORENSIC ANALYSIS ({len(win_files)} MATCHES)")
print("=" * 80)
...
print("\n" + "=" * 80)
print(f"AEGIS LATEST WINS SUMMARY ({len(win_files)} MATCHES)")
print(f"Record:               {len(win_files)} WINS, 0 LOSSES (100.0% Win Rate)")
print(f"Average Score:        ${total_my_score / len(win_files):,.0f}")
print(f"Average Margin:       +${sum(margins) / len(margins):,.0f}")
print(f"Total Margin Won:     +${sum(margins):,.0f}")
print("=" * 80)
