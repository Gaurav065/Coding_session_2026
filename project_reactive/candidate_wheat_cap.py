...
FEED_BUFFER_DAYS = 2
# Absolute ceiling on the WHEAT reserve target, independent of herd size.
# Engine FEED consumes exactly 1 WHEAT per animal per day (verified: FEED
# op does `_inv_take(inv, "WHEAT", 1)` then sets fed_today), so the single
# -day feed need for our full target herd (TARGET_COWS+TARGET_SHEEP=14) is
# 14 WHEAT. Uncapped, wheat_reserve_target = total_animals*FEED_BUFFER_DAYS
# would hit 14*2=28 at full herd, eating over a quarter of the shared
# 100-unit shed cap on a single cheap/bulky good. 16 is chosen instead of a
# tighter cap (e.g. 12) specifically because 12 < 14 would leave the reserve
# unable to cover even ONE day's feed for the full target herd -- that
# starves animals late-game and was observed to tank a validation seed via
# animal die-off, worse than the crowding bug being fixed. 16 = 14 (one full
# day at max herd) + 2 units of slack, still well under the naive 28, and
# leaves >=84 units of shed headroom for MILK/WOOL/FERTILIZER.
WHEAT_RESERVE_CAP = 16
...
def _agent_impl(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    wheat_reserve_target = min(total_animals * FEED_BUFFER_DAYS, WHEAT_RESERVE_CAP)
    ...
    for item, base in BASE_PRICES.items():
        ...
        if item == "WHEAT":
            # WHEAT above the (now-capped) reserve is always sold regardless
            # of AMM price: holding surplus wheat risks silent shed-cap
            # discard (shed is a single combined 100-unit pool shared with
            # MILK/WOOL/FERTILIZER), which is strictly worse than selling
            # cheap. Price-reactive thresholding is left unchanged for all
            # other goods.
