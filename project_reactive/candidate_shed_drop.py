...
def _actor_action(
    ...
    # 5.5. Standing on a shed-access tile while carrying sellable, non-animal
    # product (MILK/WOOL/FERTILIZER from HARVEST/COLLECT_FERTILIZER, or WHEAT
    # no longer needed for feeding this step -- steps 1-3 above already claim
    # animal deliveries and active feeding first, so anything still in `inv`
    # here is spare): DROP it into the shed immediately instead of letting it
    # sit in personal inventory until the once-daily auto-flush, where it can
    # be silently discarded once the shed's combined 100-unit cap is hit.
