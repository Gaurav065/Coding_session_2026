...
# --- Day-0 capital pacing (ISOLATED CHANGE under test) ---
# COW first_yield_day=8, SHEEP first_yield_day=6: there is a genuine
# multi-day gap with zero revenue after day 0. These two helpers only slow
# down *how fast* the agent races to spend its starting cash reaching the
# unchanged TARGET_* quotas, so a cash buffer survives that gap.
EARLY_RESERVE_BY_DAY = {0: 1200.0, 1: 700.0, 2: 300.0}


def _early_cash_reserve(day: int) -> float:
    return EARLY_RESERVE_BY_DAY.get(day, 0.0)


def _early_hand_target(day: int) -> int:
    if day <= 0:
        return min(TARGET_HANDS, 4)
    if day == 1:
        return min(TARGET_HANDS, 5)
    return TARGET_HANDS
...
def _agent_impl(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # --- 1. Morning: re-hire target crew (ramps up over the first few days
    # instead of hiring the full crew on day 0, when task load is lowest) ---
    if hour == 0:
        hand_target = _early_hand_target(day)
        needed_hands = hand_target - len(hands_pos)
    ...
    # Held off entirely on day 0 (don't buy it the same day as maxing out on
    # hires+animals); from day 1 on, still gated by a shrinking cash reserve
    # so it doesn't eat into the buffer needed before animals start yielding. ---
    ...
    early_reserve = _early_cash_reserve(day)
    if day >= 1 and len(unlocked_quads) < TARGET_QUADS and need_more_room:
        land_costs = {1: 1000, 2: 2000, 3: 4000}
        cost = land_costs.get(len(unlocked_quads), 4000)
        if money - cost >= early_reserve:
            market.append(["BUY_LAND"])
            money -= cost

    # --- 3. Buy animals if under target and cash allows (cows first).
    # Day 0 buys a smaller initial batch (max 1 per type per step instead of
    # 2) and every early purchase must leave the shrinking cash reserve
    # intact; the rest phases in over the next few days as cash allows. ---
    shed_animal_pending = {"COW": int(shed.get("COW", 0) or 0), "SHEEP": int(shed.get("SHEEP", 0) or 0)}
    max_batch = 1 if day <= 0 else 2
    for animal, target in (("COW", TARGET_COWS), ("SHEEP", TARGET_SHEEP)):
        ...
        buy_n = 0
        spend_preview = money
        while current + buy_n < target and spend_preview >= cost + 200 + early_reserve and buy_n < max_batch:
            spend_preview -= cost
            buy_n += 1
        if buy_n > 0:
