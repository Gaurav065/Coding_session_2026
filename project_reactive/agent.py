...
--- Post-ablation-study merge decision (IMPORTANT, read before touching) ---
Four fixes were validated in isolation vs "starter" (8 seeds each):
  shed_drop        avg 6467.12  (+733.50 / +12.8%)  <- BEST, kept alone below
  wheat_cap        avg 6448.12  (+714.50 / +12.5%)
  capital_phasing  avg 6351.88  (+618.25 / +10.8%)
  sell_threshold   avg 5733.63  (+0.00)             <- confirmed zero-effect, dropped
baseline (none)    avg 5733.62

A full pairwise/triple interaction sweep (all 8 subsets of {shed_drop,
wheat_cap, capital_phasing}, same 8 seeds, same harness) showed every
multi-fix combination is WORSE than the single best fix:
  shed_drop + wheat_cap                    avg 4346.50  (-1387.12 vs baseline!)
  shed_drop + capital_phasing              avg 5307.88  (-425.75)
  wheat_cap + capital_phasing              avg 5492.88  (-240.75)
  all three combined                       avg 5673.00  (-60.62, i.e. worse
                                             than doing nothing)
This is a real, deterministic interaction effect (re-run confirmed identical
results), not seed noise: each fix changes action-priority/spend-timing in a
per-step reactive agent, and stacking them shifts which actor reaches which
tile on which turn hard enough to cancel out (or reverse) each other's gains
across a 720-step game. Naively merging "every fix that individually helped"
is therefore WRONG here -- the correct merge, chosen by exhaustive subset
validation rather than assumption, is shed_drop ALONE. wheat_cap and
capital_phasing are NOT applied, despite each independently helping, because
neither composes with shed_drop (or with each other) without giving back
more than it gains elsewhere in the stack. See candidate_wheat_cap.py and
candidate_capital_phasing.py on disk for their code if revisiting this.
"""
...
def _actor_action(
    ...
    # 5.5. [shed_drop fix -- the one merged fix] Standing on a shed-access
    # tile while carrying sellable, non-animal product (MILK/WOOL/FERTILIZER
    # from HARVEST/COLLECT_FERTILIZER, or WHEAT no longer needed for feeding
    # this step -- steps 1-3 above already claim animal deliveries and active
    # feeding first, so anything still in `inv` here is spare): DROP it into
    # the shed immediately instead of letting it sit in personal inventory
    # until the once-daily auto-flush, where it can be silently discarded
    # once the shed's combined 100-unit cap is hit.
    if (x, y) in SHED_ACCESS and any(inv.get(item, 0) > 0 for item in ("MILK", "WOOL", "FERTILIZER", "WHEAT")):
        return ["DROP"]
...
def _agent_impl(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item, base in BASE_PRICES.items():
        ...
        if price >= base * SELL_PRICE_RATIO and len(market) < 10:
