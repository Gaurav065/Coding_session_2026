"""Commute & Outermost Tile Pruning Experiments — Project Maestro (§3a)

Investigates the morning commute constraint (workers respawn at shed every morning at hour 0):
1. Tile Pruning Tests:
   - Baseline: 18 SW Wheat + 6 SW Melon (24 plots), 16 NE Strawberry + 8 NE Wheat (24 plots).
   - SW Prune 4: Remove farthest SW tiles (0,9), (1,9), (0,8), (0,7) [dist 7-8 from shed].
   - SW Prune 6: Remove farthest SW tiles (0,9), (1,9), (2,9), (0,8), (1,8), (0,7) [dist 6-8].
   - NE Prune 2: Remove farthest NE tiles (9,0), (9,1) [dist 7-8 from shed].
2. Labor Allocation Tests:
   - Crew Late = 11 (give SW 3 dedicated workers instead of 2).
   - Crew Late = 12 (give SW 4 dedicated workers).

Evaluates across Self-Play (Official 20 + Disjoint 100) and H2H vs Dominant Meta (n=200).
"""
...
class CommuteOptimizedAgent(MaestroFullPortfolioAgent):
    def __init__(self,
                 sw_wheat_mode: str = "full",       # "full", "prune_4", "prune_6"
                 ne_straw_mode: str = "full",       # "full", "prune_2"
                 crew_late_override: Optional[int] = None,
                 **kwargs):
        super().__init__(**kwargs)
        self.sw_wheat_mode = sw_wheat_mode
        self.ne_straw_mode = ne_straw_mode
        self.crew_late_override = crew_late_override

        # Configure SW Wheat tiles
        if sw_wheat_mode == "prune_4":
            # Remove 4 farthest: (0,9), (1,9), (0,8), (0,7)
            pruned = {(0, 9), (1, 9), (0, 8), (0, 7)}
            self.sw_wheat = [p for p in SW_WHEAT if p not in pruned]
        elif sw_wheat_mode == "prune_6":
            # Remove 6 farthest: (0,9), (1,9), (2,9), (0,8), (1,8), (0,7)
            pruned = {(0, 9), (1, 9), (2, 9), (0, 8), (1, 8), (0, 7)}
            self.sw_wheat = [p for p in SW_WHEAT if p not in pruned]
        else:
            self.sw_wheat = list(SW_WHEAT)

        # Configure NE Strawberry tiles
        if ne_straw_mode == "prune_2":
            # Remove (9,0), (9,1)
            pruned = {(9, 0), (9, 1)}
            self.ne_strawberry = [p for p in NE_STRAWBERRY if p not in pruned]
        else:
    ...
    def __call__(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        late_crew = self.crew_late_override if self.crew_late_override is not None else self.params["crew_late"]
        if day >= 29:
            target_crew = 7
        elif day < 3:
            target_crew = 6
            ...
            if "NE" in unlocked_quads and day < 20:
                ...
                for sx, sy in self.ne_strawberry:
                    ...
                    if isinstance(t, dict) and t.get("kind") == "PLANT" and t.get("crop") == "STRAWBERRY":
                        strawberry_plants += 1
                straw_needed = max(0, len(self.ne_strawberry) - strawberry_plants
...
def _build_agent(sw_wheat_mode: str, ne_straw_mode: str, crew_late_override: Optional[int]):
    return CommuteOptimizedAgent(sw_wheat_mode=sw_wheat_mode, ne_straw_mode=ne_straw_mode, crew_late_override=crew_late_override, kw_early=10)


def _worker_match(task):
    cand_cfg, opp_kind, opp_params, seed = task
    a0 = _build_agent(*cand_cfg)
    if opp_kind == "pass":
        a1 = lambda obs: {"farmer": ["PASS"], "hands": [], "market": []}
    elif opp_kind == "dominant_meta":
        a1 = make_spatial_dispatcher_agent(params={"cow_gate_day_early": 99, "cow_gate_day_mid": 99}, kw_early=10)
    elif opp_kind == "production":
        a1 = make_spatial_dispatcher_agent(kw_early=10)
    else:
        a1 = _build_agent(*opp_kind)
...
def run_self_play(cand_cfg, seeds, max_workers=8):
    tasks = [(cand_cfg, cand_cfg, {}, s) for s in seeds]
...
def run_h2h(cand_cfg, opp_kind, seeds, max_workers=8):
    tasks = []
    for s in seeds:
        tasks.append((cand_cfg, opp_kind, {}, s))
        # Note: for symmetric execution against production / dominant meta:
        if opp_kind == "dominant_meta":
            tasks.append((("full", "full", None), "production", {}, s))  # placeholder to flip sides
        elif opp_kind == "production":
            tasks.append((("full", "full", None), cand_cfg, {}, s))
        else:
            tasks.append((opp_kind, cand_cfg, {}, s))
            
    # For clean symmetric evaluation:
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        res0 = list(ex.map(_worker_match, [(cand_cfg, opp_kind, {}, s) for s in seeds]))
        res1 = list(ex.map(_worker_match, [(("full", "full", None) if opp_kind in ("dominant_meta", "production") else opp_kind, cand_cfg if opp_kind not in ("dominant_meta", "production") else "production", {}, s) for s in seeds]))
        
    # We use a dedicated clean symmetric runner
    return


def _match_pair_worker(task):
    cand_cfg, opp_kind, seed = task
    a_cand = _build_agent(*cand_cfg)
    if opp_kind == "dominant_meta":
        a_opp = make_spatial_dispatcher_agent(params={"cow_gate_day_early": 99, "cow_gate_day_mid": 99}, kw_early=10)
    elif opp_kind == "production":
        a_opp = make_spatial_dispatcher_agent(kw_early=10)
    elif opp_kind == "pass":
        a_opp = lambda obs: {"farmer": ["PASS"], "hands": [], "market": []}
    else:
        a_opp = _build_agent(*opp_kind)
        
    # Match 0: Cand as P0
    g0 = FastGame(seed=seed)
    while not g0.done:
        g0.step_game(a_cand(g0.get_observation(0)), a_opp(g0.get_observation(1)))
    r0_cand, r0_opp = float(g0.farms[0].money), float(g0.farms[1].money)
    
    # Match 1: Opp as P0, Cand as P1
    a_cand1 = _build_agent(*cand_cfg)
    if opp_kind == "dominant_meta":
        a_opp1 = make_spatial_dispatcher_agent(params={"cow_gate_day_early": 99, "cow_gate_day_mid": 99}, kw_early=10)
    elif opp_kind == "production":
        a_opp1 = make_spatial_dispatcher_agent(kw_early=10)
    elif opp_kind == "pass":
        a_opp1 = lambda obs: {"farmer": ["PASS"], "hands": [], "market": []}
    else:
        a_opp1 = _build_agent(*opp_kind)
        
    g1 = FastGame(seed=seed)
    while not g1.done:
        g1.step_game(a_opp1(g1.get_observation(0)), a_cand1(g1.get_observation(1)))
    r1_opp, r1_cand = float(g1.farms[0].money), float(g1.farms[1].money)
    
    return (r0_cand, r0_opp, r1_cand, r1_opp)


def run_clean_h2h(cand_cfg, opp_kind, opp_label, seeds, max_workers=8):
    tasks = [(cand_cfg, opp_kind, s) for s in seeds]
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        results = list(ex.map(_match_pair_worker, tasks))
        
    diffs, prod_sc, opp_sc = [], [], []
    wins = losses = ties = 0
    for r0_c, r0_o, r1_c, r1_o in results:
...
def eval_configuration(cand_cfg, label: str, workers: int = 8):
    print(f"\n{'='*105}", flush=True)
    print(f"  {label} (Config: {cand_cfg})", flush=True)
    print(f"{'='*105}", flush=True)

    sp20_m, _, sp20_min, _ = run_self_play(cand_cfg, OFFICIAL_20, max_workers=workers)
    sp100_m, _, sp100_min, _ = run_self_play(cand_cfg, DISJOINT_100, max_workers=workers)
    ...
    r_dm = run_clean_h2h(cand_cfg, "dominant_meta", "vs Dominant Meta (10C/4S/0G, n=200)", DISJOINT_100, max_workers=workers)
    r_prod = run_clean_h2h(cand_cfg, "production", "vs Current Production (§2y, n=200)", DISJOINT_100, max_workers=workers)

    return {"label": label, "sp20": sp20_m, "sp100": sp100_m, "dm": r_dm, "prod": r_prod}


def main():
    workers = min(os.cpu_count() or 4, 8)
    print("=" * 105)
    print(f"COMMUTE & OUTERMOST TILE PRUNING EXPERIMENTS (§3a) ({workers} WORKERS)")
    print("Baseline (§2y): Official 20 = $49,777.00, Disjoint 100 = $54,692.83, DM WR = 73.4%")
    print("=" * 105)

    # 1. CANARIES
    print("\n--- RUNNING CANARIES ---", flush=True)
    r_pass = run_clean_h2h(("full", "full", None), "pass", "Canary 1: Pass Baseline", DISJOINT_100, max_workers=workers)
    ...
    r_ident = run_clean_h2h(("full", "full", None), ("full", "full", None), "Canary 2: Identity Control", DISJOINT_100, max_workers=workers)
    ...
    # 0. Baseline
    eval_configuration(("full", "full", None), "Baseline: Full SW (18 wheat + 6 melon), Full NE (16 straw + 8 wheat), Crew Late = 10", workers=workers)

    # 1. SW Prune 4 Farthest Wheat Tiles
    eval_configuration(("prune_4", "full", None), "Variant 1: SW Prune 4 Farthest Wheat Tiles (14 SW wheat)", workers=workers)

    # 2. SW Prune 6 Farthest Wheat Tiles
    eval_configuration(("prune_6", "full", None), "Variant 2: SW Prune 6 Farthest Wheat Tiles (12 SW wheat)", workers=workers)

    # 3. NE Prune 2 Farthest Strawberry Tiles
    eval_configuration(("full", "prune_2", None), "Variant 3: NE Prune 2 Farthest Strawberry Tiles (14 NE straw)", workers=workers)

    # 4. Crew Late = 11 (Dedicated 3rd SW worker)
    eval_configuration(("full", "full", 11), "Variant 4: Crew Late = 11 (Extra SW worker)", workers=workers)

    # 5. Crew Late = 12 (Dedicated 4th SW worker)
    eval_configuration(("full", "full", 12), "Variant 5: Crew Late = 12 (Two Extra SW workers)", workers=workers)

    elapsed = time.time() - t0
    print("\n" + "=" * 105, flush=True)
    print(f"Experiments completed in {elapsed:.1f}s", flush=True)
