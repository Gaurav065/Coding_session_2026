"""Direct Head-to-Head: Candidate 1 (Task Priority) vs Candidate 2 (Pure Field Retention)

Settles the decision between Cand 1 and Cand 2 at n=200 (100 Disjoint Seeds x 2 seats)
and compares their win rates against the full archetype suite.

- Cand 1 (Task Priority): No hour>=18 or cap>=15 interruption; walk to shed ONLY when idle with produce.
- Cand 2 (Pure Field Retention): Never walk to shed on days < 29; PASS in-place when idle; 100% midnight auto-flush.

Includes standing canaries (Pass Baseline and Identity Control).
"""
...
class CandAgent(MaestroFullPortfolioAgent):
    def __init__(self, mode: str = "task_first", **kwargs):
        super().__init__(**kwargs)
        self.mode = mode  # "task_first" (Cand 1) or "pure_field" (Cand 2) or "baseline" (Cand 0)
    ...
    def __call__(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for u_idx, (ux, uy) in enumerate(all_units):
            ...
            # SECTION B: CROP CREWS (UNITS 4..12)
            else:
                ...
                if action == ["PASS"]:
                    ...
                    if best_task:
                        ...
                        # Idle fallback:
                        if self.mode == "pure_field":
                            action = ["PASS"]
                        elif self.mode in ("task_first", "baseline"):
...
def _build_agent(kind: str, mode: str, opp_params: Optional[Dict[str, Any]] = None):
    if kind == "pass":
    ...
    elif kind == "wool_heavy":
        return make_spatial_dispatcher_agent(params={"cow_cap_base": 6, "sheep_cap": 12, "goose_cap": 0, "cow_gate_day_early": 99, "cow_gate_day_mid": 99}, kw_early=10)
    elif kind == "balanced":
        return make_spatial_dispatcher_agent(params={"cow_cap_base": 6, "sheep_cap": 8, "goose_cap": 0, "cow_gate_day_early": 99, "cow_gate_day_mid": 99}, kw_early=10)
    elif kind == "cand":
        return CandAgent(mode=mode, kw_early=10)
    else:
...
def run_h2h(p0_kind: str, p0_mode: str, p1_kind: str, p1_mode: str, label: str, seeds: List[int], max_workers: int = 8) -> Dict[str, Any]:
    tasks = []
    for s in seeds:
        # Seat 0: P0 is agent0, P1 is agent1
        tasks.append((p0_kind, p0_mode, p1_kind, p1_mode, s))
        # Seat 1: P1 is agent0, P0 is agent1
        tasks.append((p1_kind, p1_mode, p0_kind, p0_mode, s))
    ...
    for i in range(0, len(results), 2):
        r0_c, r1_o = results[i]
        d0 = r0_c - r1_o; diffs.append(d0); p0_sc.append(r0_c); p1_sc.append(r1_o)
        if d0 > 0: wins += 1
        elif d0 < 0: losses += 1
        else: ties += 1
        
        r0_o, r1_c = results[i+1]
        d1 = r1_c - r0_o; diffs.append(d1); p0_sc.append(r1_c); p1_sc.append(r0_o)
        if d1 > 0: wins += 1
        elif d1 < 0: losses += 1
        else: ties += 1
    ...
    mean_p0 = float(np.mean(p0_sc))
    mean_p1 = float(np.mean(p1_sc))
    
    print(f"  {label:<38} | P0: ${mean_p0:>9,.2f} | P1: ${mean_p1:>9,.2f} | Delta: ${mean_d:>+9,.2f} (t={t_stat:>+5.2f}, p={p_val:.4e}) | WR: {wr:>5.1f}% ({wins}W/{losses}L/{ties}T)", flush=True)
    return {
        "label": label, "p0_mean": mean_p0, "p1_mean": mean_p1,
...
def main():
    ...
    print("=" * 115)
    print(f"DIRECT HEAD-TO-HEAD: CANDIDATE 1 (TASK PRIORITY) vs CANDIDATE 2 (PURE FIELD RETENTION) ({workers} WORKERS)")
    print("=" * 115)

    # 1. CANARIES
    print("\n--- RUNNING CANARIES ---", flush=True)
    r_pass = run_h2h("cand", "pure_field", "pass", "", "Canary 1: Pass Baseline", DISJOINT_100, max_workers=workers)
    if abs(r_pass["p1_mean"] - 3000.0) > 1e-4 or r_pass["wr"] != 100.0:
        raise RuntimeError("CANARY 1 FAILED!")
    print("  [PASS] Canary 1 (Pass Baseline): Opponent = $3,000.00, WR = 100.0%", flush=True)

    r_ident = run_h2h("cand", "pure_field", "cand", "pure_field", "Canary 2: Identity Control", DISJOINT_100, max_workers=workers)
    if abs(r_ident["wr"] - 50.0) > 1e-4 or abs(r_ident["delta"]) > 1e-4:
        raise RuntimeError("CANARY 2 FAILED!")
    print("  [PASS] Canary 2 (Identity Control): WR = 50.0%, Delta = $0.00", flush=True)

    # 2. DIRECT HEAD-TO-HEAD MATCH (Cand 1 vs Cand 2)
    print("\n--- DIRECT COMPARISON: CANDIDATE 1 vs CANDIDATE 2 (n=200, 100 Disjoint Seeds) ---", flush=True)
    t0 = time.time()
    res_direct = run_h2h("cand", "task_first", "cand", "pure_field", "Cand 1 (Task First) vs Cand 2 (Pure Field)", DISJOINT_100, max_workers=workers)

    # 3. FULL ARCHETYPE SUITE COMPARISON
    print("\n--- FULL ARCHETYPE SUITE COMPARISON (100 Disjoint Seeds x 2 seats = 200 matches each) ---", flush=True)
    print("\nCandidate 1 (Task Priority / Drop when idle):", flush=True)
    run_h2h("cand", "task_first", "dominant_meta", "", "Cand 1 vs Dominant Meta (10C/4S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "task_first", "wool_heavy", "", "Cand 1 vs Wool-Heavy (6C/12S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "task_first", "balanced", "", "Cand 1 vs Balanced Pasture (6C/8S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "task_first", "cand", "baseline", "Cand 1 vs Prev Baseline (§2w)", DISJOINT_100, max_workers=workers)

    print("\nCandidate 2 (Pure Field Retention / PASS when idle):", flush=True)
    run_h2h("cand", "pure_field", "dominant_meta", "", "Cand 2 vs Dominant Meta (10C/4S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "pure_field", "wool_heavy", "", "Cand 2 vs Wool-Heavy (6C/12S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "pure_field", "balanced", "", "Cand 2 vs Balanced Pasture (6C/8S/0G)", DISJOINT_100, max_workers=workers)
    run_h2h("cand", "pure_field", "cand", "baseline", "Cand 2 vs Prev Baseline (§2w)", DISJOINT_100, max_workers=workers)

    elapsed = time.time() - t0
    print("\n" + "=" * 115, flush=True)
    print(f"Comparison completed in {elapsed:.1f}s", flush=True)
